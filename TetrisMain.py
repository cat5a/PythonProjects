...
class Tetris():
    FIELD_HEIGHT = 20       #Modul Variable
    FIELD_WIDTH = 10
    SCORE_PER_ELIMINATED_LINES = (0, 40, 100, 300, 1200)
    TETROMINOS = [
        [(0, 0), (0, 1), (1, 0), (1,1)], #Blok O
        [(0, 0), (0, 1), (1, 1), (2,1)], #Blok L
        [(0, 1), (1, 1), (2, 1), (2,0)], #Blok J 
        [(0, 1), (1, 0), (1, 1), (2,0)], #Blok Z
        [(0, 1), (1, 0), (1, 1), (2,1)], #Blok T
        [(0, 0), (1, 0), (1, 1), (2,1)], #Blok S
        [(0, 1), (1, 1), (2, 1), (3,1)], #Blok I
    ]
    
    def __init__(self):     #Modul Class Inheritance
    ...
    def reset_tetromino(self):      #Modul Function
    ...
    def move(self, dr, dc):
        with self.move_lock:
            if self.game_over:      #Modul Pengkondisian
                return
...
class Application(tk.Frame):        #Modul GUI
    def __init__(self, master=None):
        super().__init__(master)
        self.tetris = Tetris()      #Modul Setter getter
        self.pack()
        self.create_widgets()
        self.update_clock()
    ...
    def create_widgets(self):
        ...
        #Jadi geraknya bisa pake arrow key atau wasd ya kawan
        self.canvas.focus_set()
        self.rectangles = [
            self.canvas.create_rectangle(c*PIECE_SIZE, r*PIECE_SIZE, (c+1)*PIECE_SIZE, (r+1)*PIECE_SIZE)
                for r in range(self.tetris.FIELD_HEIGHT) for c in range(self.tetris.FIELD_WIDTH)        #Modul Looping
        ]
        self.canvas.pack(side="left")
        self.title = tk.Label(self, anchor='w', width=5, font=("System", 42), fg='green2')
    ...
    def update(self):
        ...
        self.title['text'] = "Tetris\n"
        self.status_msg['text'] = "Score: {}\nLevel: {}\nPress Arrow\nKeys or\nWASD to\nPlay".format(self.tetris.score, self.tetris.level)
